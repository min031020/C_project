...
int main() {
	...
	struct node* temp; //연결 리스트
	temp = (struct  node*)malloc(sizeof(struct node)); //연결 리스트
	...
	//키 입력 받기
	printf("키(cm) : ");
	scanf("%f", &temp->data); //구조체 cm에 저장
	fprintf(fp, "당신의 키 : %.1f\n", temp->data); //파일에 저장
	printf("\n");

	//몸무게 입력 받기
	printf("몸무게(kg) : ");
	scanf("%f", &stdata.kg); //구조체 kg에 저장
	fprintf(fp, "당신의 몸무게 : %.1f\n", stdata.kg); //파일에 저장
	printf("\n");

	abkg = ABKG(stdata.kg); // 사용자의 몸무게 대신 구간에 따른 평균 몸무게로 대체
	...
	stdata.m = temp->data / 100; //BMI 계산의 편의성을 위해 m를 구함

	BMI(stdata.m, stdata.kg, fp); //BMI계산 인자로 키와 몸무게 반환
	printf("\n");
